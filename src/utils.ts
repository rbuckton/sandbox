...
export function nodeModulePaths(dirname: string): string[] {
    return (<any>NodeJSModule)._nodeModulePaths(dirname);
}
...
export function resolveFilename(request: string, parent: NodeModule, isMain: boolean, options?: { paths?: string[]; }): string {
    return (<any>NodeJSModule)._resolveFilename(request, parent, isMain, options);
}

export function resolveLookupPaths(request: string, parent: NodeModule, newReturn?: boolean): string[] {
    return (<any>NodeJSModule)._resolveLookupPaths(request, parent, newReturn);
}

export function findPath(request: string, paths: string[], isMain = false): string | false {
    return (<any>NodeJSModule)._findPath(request, paths, isMain);
}
